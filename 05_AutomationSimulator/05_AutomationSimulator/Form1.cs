...
namespace _05_AutomationSimulator
{
    public partial class Form1 : Form
    {
        ...
        private void PositionEnabledCheckbox_CheckedChanged(object sender, EventArgs e)
        {
            if (!(PositionEnabledCheckbox as CheckBox).Checked)
            {
                CurrentWagonPosition = (IniLinksBox.Location.X + IniMitteBox.Location.X) / 2;
                Controller.Passmiddle = false;
                Timer.Stop();
            }
            else
            {
